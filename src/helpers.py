...
def datafile_structure(dtype, knot, Nbeads, pers_len):
    """Return datafile structure according to the data type
    ...
    Raises:
        Exception: return an exception if datatype is not available
    """

    # Setting up file positions
    if dtype == "XYZ":
        header = False
        fname = os.path.join("XYZ", f"XYZ_{knot}.dat.nos")
        select_cols = [0, 1, 2]
...
def generate_model(net, in_layer, knots, norm):
    ...
    Returns:
        tensorflow.keras.models: Choesen model
    """

    # Loading different networks according to the chosen setup
    if net == "FFNN":
        model = setup_NN(in_layer, len(knots), "relu", tf.keras.optimizers.Adam(learning_rate=0.001), norm)
    elif net == "FFNN2":
        model = setup_NN2(in_layer, len(knots), "relu", tf.keras.optimizers.Adam(learning_rate=0.001), norm)
    elif net == "CNN":
        model = setup_CNN(in_layer, len(knots), "relu", tf.keras.optimizers.Adam(learning_rate=0.001), norm)
    elif net == "RNN":
        model = setup_RNN(in_layer, len(knots), "tanh", tf.keras.optimizers.Adam(learning_rate=0.00001), norm)
    elif net == "RNN2":
        model = setup_RNN2(in_layer, len(knots), "tanh", tf.keras.optimizers.Adam(learning_rate=0.00001), norm)
    elif net == "RNN2b":
        model = setup_RNN2b(in_layer, len(knots), "tanh", tf.keras.optimizers.Adam(learning_rate=0.00001), norm)
    elif net == "localiseFFNN":
        model = localise_setup_NN(in_layer, len(knots), "relu", tf.keras.optimizers.Adam(learning_rate=0.001), norm)
    elif net == "localiseRNN":
        model = localise_setup_RNN(in_layer, len(knots), "tanh", tf.keras.optimizers.Adam(learning_rate=0.00001), norm)
    #elif net == "randFOR":
